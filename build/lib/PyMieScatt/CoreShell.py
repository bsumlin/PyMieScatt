...
def CoreShellScatteringFunction(mCore,mShell,wavelength,dCore,dShell,thetaSteps):
  xCore = np.pi*dCore/wavelength
  xShell = np.pi*dShell/wavelength
  theta = np.arange(thetaSteps)*np.pi/(thetaSteps-1)
  ...
  return [np.hstack((theta, theta+np.pi)),np.hstack((SL, np.flip(SR, axis=0)))]
